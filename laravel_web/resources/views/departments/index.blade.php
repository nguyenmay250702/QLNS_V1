...
@section('content')
    ...
    <div class="container-full mt-2">
        <div class="card mb-5 title">
         ...
         <div class="card">
            <div class="card-body">
                <span style="font-weight: 600;">Tìm kiếm phòng ban</span>
                <div style="border-top: 2px solid #f87729; margin-top: 5px;"></div>
            </div>

            <div class="card-body">
                <form method="get" action="{{ route('departments.search') }}" id="form_search">
                    {{-- @csrf --}}
                    <input name='function' type="text" hidden value="search">
                    <div class="d-lg-flex justify-content-between">
                        <div class="mb-3 w-100" style="align-items: center;">
                            <div class="mx-5 my-4">
                                <label class="col-label-form">Tên phòng ban:</label>
                                <div class="">
                    ...
                    <div class="text-center">
                        ...
                        <a type="button" href="{{route("departments.search")}}" class="btn btn-huy m-2">Hủy</a>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="card mt-4">
            <div class="card-body d-flex align-items-center justify-content-between">
                <div>
                    <a href="{{ route('departments.create') }}" class="btn btn-mau btn-sm" style="width: 130px;">+ Tạo mới</a>
                    <button class="btn btn-mau btn-sm" style="submit; display: inline-block; width: 130px; "
                        data-bs-toggle="modal" data-bs-target="#xuatdulieuModal"><i class="fa-solid fa-download"></i> Xuất dữ liệu</button>
                </div>
                    ...
                    @if (count($data) > 0)
                        ...
                        @foreach ($data as $row)
                            ...
                            <tr>
                                ...
                                <td>{{ $row->name }}</td>
                                <td>{{ $row->address }}</td>
                                <td>@if($row->status ==0) dừng @else hoạt động @endif</td>
                                <td class="position-relative" style="text-align: center; width: 20px">         
                                    ...
                                    <div id="list_action{{$stt}}" class="action d-none position-absolute bg-secondary-subtle ">
                                        {{-- <a href="{{ route('departments.show', $row['id']) }}" class="action_item px-3 py-2"><i class="fa-solid fa-eye"></i>xem</a> --}}
                                        <a href="{{ route('departments.edit', $row['id']) }}" class="action_item px-3 py-2"><i class="fa-solid fa-pen-to-square" style="margin-right: 11px;"></i>sửa</a>
...
@section('footer')
    <script>
        ...
        xoaModal.addEventListener('show.bs.modal', function(event) {
            ...
            form.action = "{{ route('departments.destroy', 'RECIPIENT') }}".replace('RECIPIENT', recipient);
        });
    </script>
